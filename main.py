# main.py - Enhanced with Gemini 2.5 Flash and better prompting
...
# FastAPI setup
app = FastAPI(title="Insurance Policy RAG API with Gemini 2.5 Flash", version="2.0.0")
    ...
    try:
        if gemini_model is None:
            ...
            gemini_model = genai.GenerativeModel(
                'gemini-2.0-flash-exp',  # Updated to 2.5 Flash
...
def init_pinecone():
    ...
    try:
        ...
        if index is None:
            index_name = "policy-docs-gemini-2-5-flash"  # Updated index name
            ...
            try:
                ...
                if index_name not in existing_indexes:
                    ...
                    pc.create_index(
                        name=index_name,
                        dimension=768,  # Updated dimension for better embeddings
                        metric="cosine",
                        spec=ServerlessSpec(
                            cloud="aws",
                            region="us-east-1"
...
# Text chunking with better overlap
...
# Enhanced embedding function using Gemini 2.5 Flash
def get_enhanced_embedding(text: str, gemini_model) -> List[float]:
    """Create enhanced embeddings using Gemini 2.5 Flash - EXACTLY 768 dimensions"""
    try:
        # Use Gemini 2.5 Flash to create semantic keywords and concepts
        embedding_prompt = f"""
...
"""
        ...
        logger.error(f"Error creating Gemini 2.5 Flash embedding: {e}")
...
def create_semantic_embedding(keywords: str, original_text: str) -> List[float]:
    ...
    # Basic text statistics (first 20 features)
...
def query_gemini_enhanced(question: str, context_clauses: List[str], gemini_model) -> str:
    """Enhanced query function with better prompting for direct answers"""
...
async def upsert_chunks_enhanced_async(chunks: List[str], index, gemini_model):
    """Enhanced async upsert with better embeddings"""
    try:
        batch_size = 5  # Smaller batches for API rate limiting
        logger.info(f"Processing {len(chunks)} chunks with enhanced embeddings in batches of {batch_size}")
        ...
        for i in range(0, len(chunks), batch_size):
            ...
            if batch_vectors:
                try:
                    ...
                    logger.info(f"✓ Upserted enhanced batch {i//batch_size + 1}/{(len(chunks)-1)//batch_size + 1} ({len(batch_vectors)} vectors)")
        ...
        logger.info(f"✓ Completed upserting all chunks with enhanced embeddings")
...
async def process_documents_background():
    ...
    try:
        ...
        # Upsert chunks with enhanced embeddings
        await upsert_chunks_enhanced_async(chunks, index, gemini_model)
        logger.info("✅ Policy document successfully indexed with enhanced embeddings")
...
async def startup_event():
    ...
    logger.info("=== STARTUP: Initializing enhanced services with Gemini 2.5 Flash ===")
    ...
    try:
        ...
        logger.info("🚀 Server is ready with Gemini 2.5 Flash")
...
async def run_endpoint(req: QueryRequest, verified: bool = Depends(verify_token)):
    ...
    for question in req.questions:
        try:
            logger.info(f"Processing question with Gemini 2.5 Flash: {question[:100]}...")
...
async def health_check():
    return {
        "status": "healthy", 
        "message": "Insurance Policy RAG API with Gemini 2.5 Flash",
